...
export function* postTransaction(payload: object): Generator {
    try {
        const json = yield fetch(`${BASE_URL}/transactions`, {
            method: 'POST',
            headers: {
...
export function* fetchTransactions(payload: object): Generator {
    try {
        const json = yield fetch(`${BASE_URL}/transactions/search`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
export function* fetchUserTransactions(payload: object): Generator {
    try {
        const json = yield fetch(`${BASE_URL}/transactions?userId=${payload.userId}`).then(response => response.json());
